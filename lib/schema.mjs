...
function seededTemplateOrder(templates, payload, tier) {
  const seed = `${payload?.board_key ?? "board"}|${payload?.player_uuid ?? "player"}|${payload?.player_class_id ?? "class"}|${tier}`;
...
function normalizeAiQuest(entry, byKey) {
  ...
  return {
    ...
    target_count: clampTargetCount(template, entry.target_count),
...
function baseQuestFromTemplate(template, payload, overrides = {}) {
  ...
  return {
    ...
    target_count: clampTargetCount(template, overrides.target_count ?? fallbackTargetCount(template, payload)),
