...
const TYPE_PRIORITY = {
  EASY: ["COLLECT_ITEM", "BREAK_BLOCK", "KILL_MOB", "CLASS_LEVEL"],
  MEDIUM: ["KILL_MOB", "COLLECT_ITEM", "BREAK_BLOCK", "CLASS_LEVEL"],
  HARD: ["KILL_MOB", "COLLECT_ITEM", "BREAK_BLOCK", "CLASS_LEVEL"]
};
...
function seededTemplateOrder(templates, payload, tier, extra = "") {
  const seed = `${payload?.board_key ?? "board"}|${payload?.player_uuid ?? "player"}|${payload?.player_class_id ?? "class"}|${tier}|${refreshBucket()}|${extra}`;
...
function fallbackTargetCount(template, payload) {
  ...
  const seed = `${payload?.board_key ?? "board"}|${payload?.player_uuid ?? "player"}|${template.template_key}|count|${refreshBucket()}`;
...
function targetDiversityKey(template) {
  ...
  return target.startsWith("minecraft:") ? target : `${template?.type ?? "UNKNOWN"}|${target}`;
}

function buildTemplateIndex(allowedTemplates) {
...
function chooseForTier(payload, tierTemplates, tier, globalUsedTargets) {
  const selected = [];
  const localUsedTargets = new Set();
  const order = seededTemplateOrder(tierTemplates, payload, tier);
  for (const wantedType of TYPE_PRIORITY[tier] || []) {
    const candidate = order.find(template =>
      template.type === wantedType &&
      !localUsedTargets.has(targetDiversityKey(template)) &&
      !globalUsedTargets.has(targetDiversityKey(template))
    );
    if (!candidate) continue;
    selected.push(candidate);
    localUsedTargets.add(targetDiversityKey(candidate));
    globalUsedTargets.add(targetDiversityKey(candidate));
    if (selected.length === 3) return selected;
  }
  for (const candidate of order) {
    const key = targetDiversityKey(candidate);
    if (localUsedTargets.has(key) || globalUsedTargets.has(key)) continue;
    selected.push(candidate);
    localUsedTargets.add(key);
    globalUsedTargets.add(key);
    if (selected.length === 3) return selected;
  }
  for (const candidate of order) {
    const key = targetDiversityKey(candidate);
    if (localUsedTargets.has(key)) continue;
    selected.push(candidate);
    localUsedTargets.add(key);
    if (selected.length === 3) return selected;
  }
...
export function pickTemplatesForBoard(payload) {
  const { byTier } = buildTemplateIndex(payload?.allowed_templates);
  const picked = [];
  const globalUsedTargets = new Set();
  for (const tier of TIER_ORDER) {
    const selected = chooseForTier(payload, byTier[tier], tier, globalUsedTargets);
    if (selected.length < 3) {
      throw new Error(`Not enough allowed_templates for tier ${tier}`);
    }
    picked.push(...selected.slice(0, 3));
  }
  return picked;
}
...
function normalizeAiQuest(entry, byKey) {
  ...
  const targetCount = clampTargetCount(template, entry.target_count);
  ...
  const rawRuTitle = normalizeString(entry.title_ru, fallbackTitleRu(template, targetCount));
  const rawEnTitle = normalizeString(entry.title_en, fallbackTitleEn(template, targetCount));
...
export function buildBoardFromSelection(payload, selectedTemplates, aiData) {
  const selected = Array.isArray(selectedTemplates) ? selectedTemplates : pickTemplatesForBoard(payload);
  const aiByKey = new Map();
  const index = buildTemplateIndex(payload?.allowed_templates);
  if (Array.isArray(aiData?.quests)) {
    for (const raw of aiData.quests) {
      const normalized = normalizeAiQuest(raw, index.byKey);
      if (normalized) aiByKey.set(normalized.template_key, normalized);
    }
  }
  return {
    board_id: typeof payload?.board_key === "string" && payload.board_key.trim() ? payload.board_key.trim() : undefined,
    quests: selected.map((template, index) => ({
      ...baseQuestFromTemplate(template, payload, aiByKey.get(template.template_key) || {}),
      slot: index
    }))
  };
}
...
export function validateBoard(board) {
  ...
  for (const q of board.quests) {
    ...
    if (q.tier === "EASY") easy++; else if (q.tier === "MEDIUM") medium++; else if (q.tier === "HARD") hard++; else return `Invalid tier in slot ${q.slot}: ${q.tier}`;
